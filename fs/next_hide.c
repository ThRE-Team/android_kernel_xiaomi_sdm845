...
static const char* const protected_mount_types[] = {
	"overlay"
};

static const char* const protected_mount_paths[] = {
...
static const char* const protected_mount_devices[] = {
	"KSU"
};
...
int is_protected_path(const struct path* const file)
{
	...
	for (index = 0; index < ARRAY_SIZE(protected_paths); index++) {
		const char* const name = protected_paths[index];
...
int protected_path(const struct filename* const name)
{
	int status = 0, ret = 0;
	struct path path;
	...
	if (!ret) {
		status = is_protected_path(&path);
		path_put(&path);
	}
	return status;
}

int is_protected_mount(struct vfsmount* const mnt, const struct path* const root)
{
	size_t index = 0, size = 4096;
	int res = -1, status = 0;
	char* path = NULL, *ptr = NULL, *end = NULL;
	...
	for (index = 0; index < ARRAY_SIZE(protected_mount_types); index++) {
		const char* const name = protected_mount_types[index];
	...
	for (index = 0; index < ARRAY_SIZE(protected_mount_paths); index++) {
		const char* const name = protected_mount_paths[index];
	...
	for (index = 0; index < ARRAY_SIZE(protected_mount_devices); index++) {
		const char* const name = protected_mount_devices[index];
